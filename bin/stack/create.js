...
const action = require('../../api/controllers/stack/create');
...
program.option('-f, --filename <filename>', 'Create application stack based on yaml file');

program.parse(process.argv);

let url = config.Url + '/stack/create?';
url += 'mode=json';
...
client.post(url, args, (data, response) => {
  ...
  if (data.error) {
    ...
    console.log('Application Stack create: ', data.name);
  }
});
